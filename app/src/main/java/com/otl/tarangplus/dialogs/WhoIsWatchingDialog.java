...
public class WhoIsWatchingDialog extends DialogFragment {
    ...
    public static final String TAG = WhoIsWatchingDialog.class.getSimpleName();
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            ...
            if (!PreferenceHandler.getDataSendToOTT(getActivity()) && !TextUtils.isEmpty(from_tag)
                    && from_tag.equalsIgnoreCase(RegisterFragment.TAG) || from_tag.equalsIgnoreCase(OtpScreen.TAG)) {
                ...
                try {
                   ...
                   // Log.e("just smallupdate", "onInstallReferrerSetupFinished: " + PreferenceHandler.getReferalDetails(getActivity()).toString());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            ...
            if (!TextUtils.isEmpty(from_tag) && from_tag.equalsIgnoreCase(RegisterFragment.TAG) || from_tag.equalsIgnoreCase(OtpScreen.TAG))
        ...
        proceed.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (mListItems != null && mListItems.size() > 0) {
                    ...
                    try {
                        if (getArguments() != null){
                            from_tag = getArguments().getString(Constants.FROM_PAGE);
                        }
                    }catch (Exception e){
                        from_tag = "";
                    }
                    if(PreferenceHandler.getIsFreeTrail(getActivity())
                            && !TextUtils.isEmpty(from_tag) && from_tag.equalsIgnoreCase(OtpScreen.TAG) ){
                        FreePackDialog freePackDialog = new FreePackDialog();
                        freePackDialog.show(getFragmentManager(), FreePackDialog.TAG);
                    }else{
    ...
    private void showCongratulationsDialog() {
        ...
        type = "Your account has been successfully created";
...
//        GradientTextView cancel = (GradientTextView) dialog.findViewById(R.id.cancel);
        ...
        MyTextView txtDes = (MyTextView) dialog.findViewById(R.id.txt_des);
        txtDes.setText(type);
...
//                mWhoIsWatchingItemViewHolder.mNameImageText.setTextColor(ContextCompat.getColor(getActivity(), R.color.profile_txt_grey));
                mWhoIsWatchingItemViewHolder.mNameImageText.setBackground(getActivity().getDrawable(R.drawable.new_profile));
                ...
                if (getArguments() != null) {
                    ...
                    if (!TextUtils.isEmpty(from_tag)) {
                        if (!from_tag.equalsIgnoreCase(RegisterFragment.TAG)) {
                            mWhoIsWatchingItemViewHolder.mFullView.setVisibility(View.GONE);
                        }

                    }
                    if (!TextUtils.isEmpty(whoInitiated)) {
                        if (whoInitiated.equalsIgnoreCase(OtpScreen.TAG)) {
                            mWhoIsWatchingItemViewHolder.mFullView.setVisibility(View.VISIBLE);
                        }
                    }
                }
                ...
                mWhoIsWatchingItemViewHolder.mNameImageText.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        if (getArguments() != null) {
                            ...
                            if (!TextUtils.isEmpty(from_tag)) {
                                if (from_tag.equalsIgnoreCase(OtpScreen.TAG))
                                    return;
                                else if (from_tag.equalsIgnoreCase(RegisterFragment.TAG))
                                    return;
                                else {
                                    if (item.isChild())
                                        changeUser(item);
        ...
        private void setTextAccordingToName(ViewHolder holder, Profile profile) {
            if (profile.getFirstname() != null && profile.getLastname() != null && profile.getFirstname().trim().length() > 0 && profile.getLastname().trim().length() > 0) {
                ...
                if (profile.getFirstname().indexOf(" ") == 0 || profile.getFirstname().indexOf(" ") == profile.getFirstname().length() - 1) {
                    holder.mNameImageText.setText(profile.getFirstname().trim());

                } else if (profile.getFirstname().split(" ").length > 1) {
